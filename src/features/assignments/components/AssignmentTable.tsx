...
import { ChevronLeft, ChevronRight, Clock, MapPin, MoreVertical, UserPlus, RefreshCw, Eye } from 'lucide-react';
...
export function AssignmentTable({ 
  ...
  const mapVariant = (status: string) => {
    if (status === 'WAITING_FOR_ASSIGNMENT') return 'waiting';
    if (status === 'ASSIGNED') return 'assigned';
    if (status === 'REASSIGNED') return 'warning';
    if (['DELIVERED', 'COMPLETED'].includes(status)) return 'success';
    return 'active';
  };

  const getStatusDisplay = (order: DeliveryOrder) => {
    if (order.assignmentHistory && order.assignmentHistory.length > 1 && order.status !== 'DELIVERED') {
      return 'REASSIGNED';
    }
    return order.status;
  };
    ...
    <div style={{ backgroundColor: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr>
              ...
              <th style={{ padding: 'var(--spacing-4)', fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', fontWeight: 500, borderBottom: '1px solid var(--color-border)', backgroundColor: 'var(--color-background)' }}>Assigned At</th>
                ...
                >
                  ...
                  <td style={{ padding: 'var(--spacing-4)' }}>
                    <StatusBadge 
                      ...
                      variant={mapVariant(displayStatus)}
                    />
                  </td>
                  <td style={{ padding: 'var(--spacing-4)' }}>
                    {order.assignedAt ? (
                  ...
                  <td style={{ padding: 'var(--spacing-4)', textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 'var(--spacing-2)', position: 'relative' }}>
                      {order.status === 'WAITING_FOR_ASSIGNMENT' ? (
                        <Button size="sm" onClick={() => onAssignClick(order)}>
      ...
      <div style={{ padding: 'var(--spacing-4)', borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'var(--color-background)' }}>
        <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>
          Showing {(page - 1) * limit + 1}-{Math.min(page * limit, total)} of {total} assignments
        </div>
